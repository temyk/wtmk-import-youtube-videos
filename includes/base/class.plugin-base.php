...
abstract class Plugin_Base {
	...
	public function getOption( string $option_name, $default_value = '' ) {
		$option = get_option( WYV_PLUGIN_PREFIX . '_' . $option_name );
	...
	public function addOption( string $option_name, $option_value ): bool {
		return add_option( WYV_PLUGIN_PREFIX . '_' . $option_name, $option_value );
	}
	...
	public function updateOption( string $option_name, $option_value ): bool {
		return update_option( WYV_PLUGIN_PREFIX . '_' . $option_name, $option_value );
	}
	...
	public function deleteOption( string $option_name ): bool {
		return delete_option( WYV_PLUGIN_PREFIX . '_' . $option_name );
	}
	...
	public function render_template( $template_name, $args = [] ) {
		$template_name = apply_filters( WYV_PLUGIN_PREFIX . '/template/name', $template_name, $args );

		$path = WYV_PLUGIN_DIR . "/templates/$template_name.php";
		if ( file_exists( $path ) ) {
			ob_start();
			include $path;

			echo apply_filters( WYV_PLUGIN_PREFIX . '/content/template', ob_get_clean(), $template_name, $args ); // @codingStandardsIgnoreLine
		} else {
			echo apply_filters( WYV_PLUGIN_PREFIX . '/message/template_not_found', __( 'This template does not exist!', 'wtmk-import-youtube-videos' ) ); // @codingStandardsIgnoreLine
		}
	}
}
